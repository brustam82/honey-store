...
def product_kb(name: str, vol: str, qty: int, category: str):
    ...
    Интерактивная карточка товара:
      Ряд 1: кнопки выбора объёма (1кг / 1л) — активная отмечена ✅
      Ряд 2: управление количеством  ➖  [N]  ➕
      Ряд 3: добавить в корзину
      Ряд 4: назад к списку товаров
    """
...
async def add_to_cart(call: types.CallbackQuery):
    ...
    for item in cart:
        if item["name"] == item_key:
            ...
            return
    cart.append({"name": item_key, "vol": vol_label, "qty": qty, "price": price})
...
async def _finalize(m: types.Message, state: FSMContext, phone: str):
    ...
    if ok:
        ...
        await m.answer("❌ Xatolik yuz berdi. Qayta urinib ko'ring. / Произошла ошибка. Попробуйте снова.",
                       reply_markup=main_kb())
